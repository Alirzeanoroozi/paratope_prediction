...
def residue_seq_to_one(seq):
    three_to_one = lambda r: Polypeptide.three_to_one(r.resname) if r.resname in Polypeptide.standard_aa_names else 'X'
...
def produce_annotated_ab_structure(ab_h_chain, ab_l_chain, ab_seq, probs):
    ab_h_chain = annotate_chain_with_prob(ab_h_chain, ab_seq["H"], "H", probs[0:3, :])
    ab_l_chain = annotate_chain_with_prob(ab_l_chain, ab_seq["L"], "L", probs[3:6, :])
