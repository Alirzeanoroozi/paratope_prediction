...
from predict import get_dataloader, evaluate_model
...
parser = argparse.ArgumentParser(prog="Paragraph", description=description, epilog=epilogue, formatter_class=argparse.RawDescriptionHelpFormatter)
parser.add_argument("-e", "--example", help="run Paragraph using default weights on example data", action='store_true', default=False)
parser.add_argument("-k", "--pdb_H_L_csv", help="abspath to csv key containing names of pdb files (without .pdb), and H & L chain IDs", default=None)
...
parser.add_argument("-w", "--weights", help="abspath to weights to be used by network (default is pre-trained paired)", default=None)
parser.add_argument("-H", "--heavy", help="use pre-trained weights for network trained on heavy chain only (ensure key does not have L chain e.g. 4edw,H,)", action='store_true', default=False)
parser.add_argument("-L", "--light", help="use pre-trained weights for network trained on light chain only (ensure key does not have H chain e.g. 4edw,,L)", action='store_true', default=False)
